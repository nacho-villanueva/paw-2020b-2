...
public class OrderJpaDao implements OrderDao {
    ...
    @Override
    public Collection<StudyType> getRelevantStudyTypes(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic, int page, int pageSize) {

        if (user == null)
            return new ArrayList<>();

        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<StudyType> cr = cb.createQuery(StudyType.class);
        Root<Order> orderRoot = cr.from(Order.class);
        Root<StudyType> studyTypeRoot = cr.from(StudyType.class);

        Collection<Predicate> predicates = new ArrayList<>();
        predicates.add(cb.equal(orderRoot.get("study"), studyTypeRoot.get("id")));
        predicates = getAllAsUserQueryPredicate(predicates, orderRoot, user, includeSharedIfMedic);
        predicates = getFilteredPredicate(predicates, orderRoot, clinicUsers, medicUsers, patientEmails, fromDate, toDate, types);

        cr.select(studyTypeRoot).distinct(true);

        cr.where(predicates.toArray(new Predicate[0]));

        cr.orderBy(cb.asc(studyTypeRoot.get("id")));

        final TypedQuery<StudyType> query = em.createQuery(cr);
    ...
    @Override
    public Collection<Clinic> getRelevantClinics(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic, int page, int pageSize) {
        ...
        Root<Order> orderRoot = cr.from(Order.class);
        Root<Clinic> clinicRoot = cr.from(Clinic.class);

        Collection<Predicate> predicates = new ArrayList<>();
        predicates.add(cb.equal(orderRoot.get("clinic").get("user").get("id"), clinicRoot.get("user").get("id")));
        predicates = getAllAsUserQueryPredicate(predicates, orderRoot, user, includeSharedIfMedic);
        predicates = getFilteredPredicate(predicates, orderRoot, clinicUsers, medicUsers, patientEmails, fromDate, toDate, types);

        cr.select(clinicRoot).distinct(true);

        cr.where(predicates.toArray(new Predicate[0]));

        cr.orderBy(cb.asc(clinicRoot.get("user").get("id")));

        final TypedQuery<Clinic> query = em.createQuery(cr);
    ...
    @Override
    public Collection<Medic> getRelevantMedics(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic, int page, int pageSize) {

        if (user == null)
            return new ArrayList<>();

        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Medic> cr = cb.createQuery(Medic.class);
        Root<Order> orderRoot = cr.from(Order.class);
        Root<Medic> medicRoot = cr.from(Medic.class);

        Collection<Predicate> predicates = new ArrayList<>();
        predicates.add(cb.equal(orderRoot.get("medic").get("user").get("id"), medicRoot.get("user").get("id")));
        predicates = getAllAsUserQueryPredicate(predicates, orderRoot, user, includeSharedIfMedic);
        predicates = getFilteredPredicate(predicates, orderRoot, clinicUsers, medicUsers, patientEmails, fromDate, toDate, types);

        cr.select(medicRoot).distinct(true);

        cr.where(predicates.toArray(new Predicate[0]));

        cr.orderBy(cb.asc(medicRoot.get("user").get("id")));

        final TypedQuery<Medic> query = em.createQuery(cr);
    ...
    @Override
    public Collection<String> getRelevantPatientEmails(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic, int page, int pageSize) {
        ...
        cr.select(orderRoot.get("patientEmail")).distinct(true);

        cr.where(predicates.toArray(new Predicate[0]));

        cr.orderBy(cb.asc(orderRoot.get("patientEmail")));

        final TypedQuery<String> query = em.createQuery(cr);
    ...
    @Override
    public long getRelevantStudyTypesCount(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic) {
        ...
        cr.select(cb.countDistinct(studyTypeRoot));
    ...
    @Override
    public long getRelevantClinicsCount(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic) {
        ...
        cr.select(cb.countDistinct(clinicRoot));
    ...
    @Override
    public long getRelevantMedicsCount(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic) {
        ...
        cr.select(cb.countDistinct(medicRoot));
    ...
    @Override
    public long getRelevantPatientEmailsCount(User user, Collection<User> clinicUsers, Collection<User> medicUsers, Collection<String> patientEmails, LocalDate fromDate, LocalDate toDate, Collection<StudyType> types, boolean includeSharedIfMedic) {
        ...
        cr.select(cb.countDistinct(orderRoot.get("patientEmail"))).distinct(true);
